...
"$1_$2").replace(/-/g,"_").toLowerCase()},dasherize:function(){return this.replace(/_/g,"-")},inspect:function(d){var e=this.replace(/[\x00-\x1f\\]/g,function(f){if(f in String.specialChar)return String.specialChar[f];return"\\u00"+f.charCodeAt().toPaddedString(2,16)});if(d)return'"'+e.replace(/"/g,'\\"')+'"';return"'"+e.replace(/'/g,"\\'")+"'"},toJSON:function(){return this.inspect(true)},unfilterJSON:function(d){return this.replace(d||Prototype.JSONFilter,"$1")},isJSON:function(){var d;if(this.blank())return false;
...
any:b,some:b,collect:c,map:c,detect:d,findAll:e,select:e,filter:e,grep:function(g,i,j){i=i||Prototype.K;var l=[];if(Object.isString(g))g=new RegExp(RegExp.escape(g));this.each(function(n,q){g.match(n)&&l.push(i.call(j,n,q))});return l},include:f,member:f,inGroupsOf:function(g,i){i=Object.isUndefined(i)?null:i;return this.eachSlice(g,function(j){for(;j.length<g;)j.push(i);return j})},inject:function(g,i,j){this.each(function(l,n){g=i.call(j,g,l,n)});return g},invoke:function(g){var i=$A(arguments).slice(1);
...
f;c=d[f];if(e.values[f])b=e.values[f](a,c);if(c===false||c===null)a.removeAttribute(b);else c===true?a.setAttribute(b,b):a.setAttribute(b,c)}return a},getHeight:function(a){return Element.getDimensions(a).height},getWidth:function(a){return Element.getDimensions(a).width},classNames:function(a){return new Element.ClassNames(a)},hasClassName:function(a,b){if(a=$(a)){var c=a.className;return c.length>0&&(c==b||(new RegExp("(^|\\s)"+b+"(\\s|$)")).test(c))}},addClassName:function(a,b){if(a=$(a)){Element.hasClassName(a,
...
if(!document.getElementsByClassName)document.getElementsByClassName=function(a){function b(c){return c.blank()?null:"[contains(concat(' ', @class, ' '), ' "+c+" ')]"}a.getElementsByClassName=Prototype.BrowserFeatures.XPath?function(c,d){d=d.toString().strip();var e=/\s/.test(d)?$w(d).map(b).join(""):b(d);return e?document._getElementsByXPath(".//*"+e,c):[]}:function(c,d){d=d.toString().strip();var e=[],f=/\s/.test(d)?$w(d):null;if(!f&&!d)return e;for(var h=$(c).getElementsByTagName("*"),g=0,i,j;i=
h[g];g++)if(i.className&&(j=" "+i.className+" ")&&(j.include(" "+d+" ")||f&&f.all(function(l){return!l.toString().blank()&&j.include(" "+l+" ")})))e.push(Element.extend(i));return e};return function(c,d){return $(d||document.body).getElementsByClassName(c)}}(Element.Methods);Element.ClassNames=Class.create();
